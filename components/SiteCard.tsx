  ...
  return (
    <article className={`card${confirming ? " is-confirming" : ""}${hidden ? " is-hidden" : ""}`} data-card data-reveal data-id={site.id}>
          ...
          <div className="card__media">
            {site.image ? (
              ...
              <div className="card__placeholder" aria-label="Nog geen snapshot">
                ...
                <span className="card__placeholder-note">Nog geen snapshot. Deze site houdt robots buiten, upload er zelf een.</span>
              </div>
            )}
          </div>
      ...
      <div className="card__tools">
        {confirming ? (
          <>
            <button type="button" className="tool tool--confirm" onClick={() => onDelete(site)}>
              <IconCheck size={13} stroke={2.4} /> Verwijderen
            </button>
            <button type="button" className="tool" onClick={() => setConfirming(false)} aria-label="Annuleren">
          ...
          <>
            ...
            <button type="button" className="tool tool--danger" onClick={askDelete} title="Verwijderen" aria-label="Verwijderen">
              <IconX size={13} stroke={2.2} />
            </button>
            <input
              ref={fileRef}
          ...
          {error ? (
            <>
              ...
              <div className="card__status-actions">
                <button type="button" className="tool" onClick={() => onRetry(item)}>
                  <IconRefresh size={13} stroke={2.2} /> Opnieuw
                </button>
                <button type="button" className="tool" onClick={() => onDismiss(item)} aria-label="Sluiten">
                  <IconX size={13} stroke={2.2} />
                </button>
              </div>
            </>
